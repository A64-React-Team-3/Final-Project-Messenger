...
const Message: React.FC<MessageProps> = ({ message }): JSX.Element => {
  ...
  return (
    <>
      <div className={`chat ${message.sender === user!.uid ? "chat-end" : "chat-start"} mt-5 mb-2 p-0 relative`} onMouseEnter={() => setIsPickerVisible(true)} onMouseLeave={() => setIsPickerVisible(false)}>
        ...
        {isPickerVisible && <div className={`absolute z-10 top-[-2rem] ${message.sender === user!.uid ? "left-[50rem]" : "right-[50rem]"}`} style={{ transform: 'scale(0.85)' }}>
          <EmojiPicker
      ...
      <div className={`flex gap-2 z-10 bottom-[-1rem] w-50 flex-wrap ${message.sender === user!.uid ? "left-[70rem] flex-row-reverse" : "right-[70rem]"}`}>{message.reactions?.map(
        (reaction, index) => (
          <span key={index} className="text-xs">{reaction.name} {reaction.count}</span>
        )
      )}</div>
    </>
  );
